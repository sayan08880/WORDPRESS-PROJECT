...
class CRT_Breadcrumbs extends Widget_Base {
	
	public function get_name() {
		return 'crt-breadcrumbs';
	}

	public function get_title() {
		return esc_html__( 'Breadcrumbs', 'crt-manage' );
	}
	...
	protected function register_controls() {
		...
		$this->add_responsive_control(
            ...
            [
				...
				'selectors' => [
					'{{WRAPPER}} .crt-post-breadcrumbs' => '{{VALUE}}',
					'{{WRAPPER}} .crt-breadcrumbs' => '{{VALUE}}'
				],
				'separator' => 'before'
            ]
        );
		...
		$this->add_control(
			...
			[
				...
				'default' => '#787878',
				'selectors' => [
					'{{WRAPPER}} .crt-post-breadcrumbs' => 'color: {{VALUE}}',
					'{{WRAPPER}} .crt-post-breadcrumbs a' => 'color: {{VALUE}}',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'default' => '#e55b5b',
				'selectors' => [
					'{{WRAPPER}} .crt-post-breadcrumbs a:hover' => 'color: {{VALUE}}',
				],
			]
		);
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .crt-post-breadcrumbs',
				'fields_options' => [
					'typography' => [
						'default' => 'custom',
					],
	...
	protected function render() {
		...
		// Output
		echo '<div class="crt-post-breadcrumbs">';
	...
	public function post_breadcrumbs( $show_home, $separator ) {
		global $post;
		$breadcrumb = '<ul class="crt-breadcrumbs">';
		...
		if ( is_singular() ) {
			...
			if ( 'product' === $post->post_type && function_exists( 'wc_get_product_terms' ) ) {
				...
				if ( $product_categories ) {
					...
					$breadcrumb .= $this->crt_get_category_hierarchy( $main_category->term_id, $separator, 'product_cat' );
				}
				...
				if ( $categories && ! empty( $categories ) ) {
					...
					$breadcrumb .= $this->crt_get_category_hierarchy( $categories[0]->term_id, $separator, 'category' );
				}
			}
...
//        }
		...
		elseif ( Utilities::is_blog_archive() ) {
			$breadcrumb .= '<li>' . esc_html__( get_the_title(get_option('page_for_posts')), 'crt-manage' ) . '</li>';
		}
	...
	public function crt_get_category_hierarchy( $category_id, $separator, $taxonomy = 'category' ) {
		$category_chain = '';
		...
		if ( $current_category && ! is_wp_error( $current_category ) ) {
			...
			if ( $current_category->parent ) {
				$category_chain = $this->crt_get_category_hierarchy( $current_category->parent, $separator, $taxonomy ) . $category_chain;
			}
		}
