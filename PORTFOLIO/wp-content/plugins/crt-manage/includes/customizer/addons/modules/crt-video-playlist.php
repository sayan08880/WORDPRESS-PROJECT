...
class CRT_Video_Playlist extends Widget_Base {

	public function get_name() {
		return 'crt-video-playlist';
	}

	public function get_title() {
		return esc_html__( 'Video Playlist', 'crt-manage' );
	}

	public function get_icon() {
		return 'crt-icon eicon-video-playlist';
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			'playlist_query',
			[
				'label' => esc_html__( 'Playlist Query', 'crt-manage' ),
				'type' => \Elementor\Controls_Manager::SELECT,
				'default' => 'custom',
				'options' => [
					'custom' => esc_html__( 'Custom URLs', 'crt-manage' ),
					'playlist' => esc_html__( 'YouTube Playlist', 'crt-manage' ),
				],
			]
		);
		...
		$this->add_control(
			'youtube_api_key',
			[
				'label' => esc_html__( 'YouTube API Key', 'crt-manage' ),
		...
		$this->add_control(
			'youtube_playlist_id',
			[
				'label' => esc_html__( 'YouTube Playlist ID', 'crt-manage' ),
				'type' => \Elementor\Controls_Manager::TEXT,
				'default' => '',
        ...
        $this->add_control(
            'playlist_title',
            [
                'label' => esc_html__( 'Playlist Title', 'crt-manage' ),
                'type' => \Elementor\Controls_Manager::TEXT,
                'default' => esc_html__( 'Now Playing', 'crt-manage' ),
            ]
        );
        ...
        $this->add_control(
            'urls_group_title',
            [
                'label' => esc_html__( 'Custom Video URLs', 'crt-manage' ),
                'type' => \Elementor\Controls_Manager::HEADING,
                'separator' => 'before',
            ]
        );
        ...
        $this->add_control(
            'video_urls',
            [
                'label' => esc_html__( 'Video URLs', 'crt-manage' ),
		...
		$this->add_control(
			'widget_accent_color',
			[
				'label' => esc_html__( 'Accent Color', 'crt-manage' ),
				'type' => \Elementor\Controls_Manager::COLOR,
                'selectors' => [
                    '{{WRAPPER}} .crt-vplaylist-controller' => 'background-color: {{VALUE}};',
                ],
				'default' => '#f84643',
			]
		);
        ...
        $this->add_group_control(
            ...
            [
                ...
                'selector' => '{{WRAPPER}} .crt-vplaylist-current-title, {{WRAPPER}} .crt-vplaylist-info-title',
				'fields_options' => [
					'typography' => [
						'default' => 'custom',
					],
    ...
    public function crt_get_svg_icon( $icon ) {
        ...
        $output  = '<span class="crt-svg-icon crt-inline-flex">';
    ...
    protected function render() {
        ...
        $playlist_title  = !empty($settings['playlist_title']) ? $settings['playlist_title'] : esc_html__('Now Playing', 'crt-manage');
        ...
        echo '<div class="crt-vplaylist-wrap">';

            // Video Player
            echo '<div class="video-player-wrap">';
                echo '<div class="video-player">';
                    echo '<div class="crt-vplaylist-main"></div>';
                echo '</div>';
            echo '</div>';

            // Playlist Panel
            echo '<div class="crt-vplaylist-thumbs-wrap">';

                echo '<div class="crt-vplaylist-highlight">';
                    echo '<div class="crt-vplaylist-heading">';
                        echo '<span>' . esc_html($playlist_title) . '</span>';
                        echo '<' . esc_attr($title_tag) . ' class="crt-vplaylist-current-title"></' . esc_attr($title_tag) . '>';
                    echo '</div>';

                    echo '<div class="crt-vplaylist-controller">';
                        echo '<div class="crt-play">' . $this->crt_get_svg_icon('play') . '</div>';
                        echo '<div class="crt-pause">' . $this->crt_get_svg_icon('pause') . '</div>';
                    echo '</div>';
                echo '</div>';

                echo '<div class="crt-vplaylist-thumbs" data-urls=\'' . esc_attr(wp_json_encode($video_urls)) . '\'>';
                    echo '<ul></ul>';
                echo '</div>';
