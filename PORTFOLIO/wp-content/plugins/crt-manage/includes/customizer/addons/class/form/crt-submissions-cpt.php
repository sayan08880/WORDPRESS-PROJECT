 ...
 */
 ...
 class CRT_Form_Builder_Submissions {

    public function __construct() {
        add_action('wp_ajax_crt_form_builder_submissions' , [$this, 'add_to_submissions']);
        add_action('wp_ajax_nopriv_crt_form_builder_submissions',[$this, 'add_to_submissions']);
...
//        }
        ...
        foreach ($_POST['form_content'] as $key => $value ) {
            ...
            $label = $this->crt_sanitize_form_field( $type, $value[1] );
        ...
        update_post_meta($post_id, 'crt_form_name', $sanitized_form_name);
        update_post_meta($post_id, 'crt_form_id', $sanitized_form_id);
        update_post_meta($post_id, 'crt_form_page', $sanitized_form_page);
        update_post_meta($post_id, 'crt_form_page_id', $sanitized_form_page_id);
        update_post_meta($post_id, 'crt_user_agent', sanitize_textarea_field( wp_unslash( $_SERVER['HTTP_USER_AGENT'] ) ));
        update_post_meta($post_id, 'crt_user_ip', Utilities::get_client_ip());
        
        if( $post_id ) {
            wp_send_json_success(array(
                'action' => 'crt_form_builder_submissions',
                'post_id' => $post_id,
                'message' => esc_html__('Submission created successfully', 'crt-addons'),
				'status' => 'success',
            ...
            wp_send_json_success(array(
                'action' => 'crt_form_builder_submissions',
                'post_id' => $post_id,
                'message' => esc_html__('Submit action failed', 'crt-addons'),
				'status' => 'error'
            ));
        }
    }
    ...
    public function update_submissions_post_meta($post_id) {
        ...
        if ( isset($_POST['crt_submission_changes']) && ! empty($_POST['crt_submission_changes']) ) {
            $changes = json_decode(stripslashes($_POST['crt_submission_changes']), true);
    ...
    public function crt_sanitize_form_field( $type, $value ) {
        // if ( is_array( $value ) ) {
        //     return array_map( function( $item ) use ( $type ) {
        //         return $this->crt_sanitize_form_field( $type, $item );
        //     }, $value );
        // }
 ...
 new CRT_Form_Builder_Submissions();