...
/**
 * CRT_Woo_Grid_Helpers setup
 *
 * @since 3.4.6
 */

 class CRT_Woo_Grid_Helpers {

    public function __construct() {
		add_action('wp_ajax_crt_woo_grid_filters_ajax', [$this, 'crt_woo_grid_filters_ajax']);
		add_action('wp_ajax_nopriv_crt_woo_grid_filters_ajax', [$this, 'crt_woo_grid_filters_ajax']);
		add_action('wp_ajax_crt_get_filtered_count_products', [$this, 'crt_get_filtered_count_products']);
		add_action('wp_ajax_nopriv_crt_get_filtered_count_products', [$this, 'crt_get_filtered_count_products']);
    }
	...
	public static function get_max_num_pages( $settings ) {
		if ( isset($_POST['crt_url_params']) ) {
			$query = new \WP_Query( CRT_Woo_Grid_Helpers::get_main_query_args($settings, []) );
...
//		}
		...
		$args = [
			'post_type' => 'product',
			'tax_query' => CRT_Woo_Grid_Helpers::get_tax_query_args($settings),
			'meta_query' => CRT_Woo_Grid_Helpers::get_meta_query_args(),
			'post__not_in' => isset($settings[ 'query_exclude_products' ]) ? $settings[ 'query_exclude_products' ]:'',
		...
		if ( 'upsell' === $settings['query_selection'] ) {
			...
			CRT_Woo_Grid_Helpers::$my_upsells = $product->get_upsell_ids();
			
			if ( !empty(CRT_Woo_Grid_Helpers::$my_upsells) ) {
				$args = array(
					'post_type' => 'product',
					...
					'paged' => $paged,
					'post__in' => CRT_Woo_Grid_Helpers::$my_upsells,
		...
		if ( 'cross-sell' === $settings['query_selection'] ) {
			// Get Product
			CRT_Woo_Grid_Helpers::$crossell_ids = [];
			...
			if( is_cart() ) {
				...
				foreach($items as $item => $values) {
					...
					foreach($cross_sell_products as $cs_product) {
						array_push(CRT_Woo_Grid_Helpers::$crossell_ids, $cs_product);
					}
				  }
			}
			...
			if ( is_single() ) {
				...
				CRT_Woo_Grid_Helpers::$crossell_ids = $product->get_cross_sell_ids();
			}
	
			// $meta_query = WC()->query->get_meta_query();
			
			if ( !empty(CRT_Woo_Grid_Helpers::$crossell_ids) ) {
				$args = [
					'post_type' => 'product',
					'post__not_in' => $settings[ 'query_exclude_products' ],
					'tax_query' => CRT_Woo_Grid_Helpers::get_tax_query_args($settings),
					...
					'order' => $settings['order_direction'],
					'paged' => $paged,
					'post__in' => CRT_Woo_Grid_Helpers::$crossell_ids,
					// 'meta_query' => $meta_query
				];
			} else {
				$args['post_type'] = 'none';
        ...
        if(isset($settings['query_exclude_out_of_stock'])) {
		...
		if ( 'current' === $settings[ 'query_selection' ] && true !== \Elementor\Plugin::$instance->editor->is_edit_mode() ) {
			...
			if ( is_product_category() ) {
				$posts_per_page = intval(get_option('crt_woo_shop_cat_ppp', 9));
			} elseif ( is_product_tag() ) {
				$posts_per_page = intval(get_option('crt_woo_shop_tag_ppp', 9));
			} else {
				$posts_per_page = intval(get_option('crt_woo_shop_ppp', 9));
			}
//			$args = $wp_query->query_vars;
			$args = array();
			$args['tax_query'] = CRT_Woo_Grid_Helpers::get_tax_query_args($settings);
			$args['meta_query'] = CRT_Woo_Grid_Helpers::get_meta_query_args();
			$args['posts_per_page'] = $posts_per_page;
			if (!empty($settings['query_randomize'])) {
				$args['orderby'] = $settings['query_randomize'];
			} else {
                $settings['current_query_orderby'] = isset($settings['current_query_orderby']) ? $settings['current_query_orderby']:'';
...
//					if (strpos($key, 'crt_af_') !== false) {
                    if (strpos($key, 'crt_af_') !== false || strpos($key, 'attr-') !== false || $key == 'filter_price' || taxonomy_exists($key)) {
                        // Need to setup logic to get relation from filters separately
						$cleanedKey = str_replace('crt_af_', '', $key);
                        $cleanedKey = str_replace('attr-', '', $cleanedKey);
                        if ($key == 'filter_price') {
                            $cleanedKey = 'price';
                        }
						...
						if ( isset($crt_url_params[$key]) ) {
							if ( $cleanedKey == 'date_range' ) {
								...
								$selected_ratings = explode( ',', $crt_url_params[$key] );
								...
								$meta_query[] = $rating_meta_query;
									...
									if (strpos($crt_url_params[$key], '-') !== false) {
										$price_range = explode('-', $crt_url_params[$key]);
									} elseif (strpos($crt_url_params[$key], ',') !== false) {
										$price_range = explode(',', $crt_url_params[$key]);
									}
								...
								if ( $crt_url_params[$key] != '0' ) {
									...
									if ( str_contains($crt_url_params[$key], ',') ) {
										...
										// Example usage
										$key_type = CRT_Woo_Grid_Helpers::identify_key_type($cleanedKey);
										...
										if ( ('meta_field' == $key_type || 'custom_field' == $key_type) ) {
											if ( is_numeric($filtervalues[0]) && isset($crt_url_params['crt_aft_' . $cleanedKey]) && $crt_url_params['crt_aft_' . $cleanedKey] == 'range' ) {
												...
												if ( isset($meta_query) ) {
													if ( isset($crt_url_params['crt_afr_' . $cleanedKey]) && !empty(explode(',', $crt_url_params['crt_afr_'. $cleanedKey])[0]) ) {
    ...
    public static function get_meta_query_args(){
     ...
     public static function get_tax_query_args($settings) {
         ...
         if ( isset($_GET['crtfilters']) ) {
             ...
             if ( isset($_GET['crt_select_product_cat']) ) {
                 if ( $_GET['crt_select_product_cat'] != '0' ) {
                     // Get category from URL
                     $category = sanitize_text_field($_GET['crt_select_product_cat']);
             ...
             if ( isset($_POST['crt_taxonomy']) ) {
                 ...
                 if ( $term != '*' ) {
                     if ( 'tag' === $taxonomy ) {
                         $taxonomy = 'product_' . $_POST['crt_taxonomy'];
                     }
                     array_push( $tax_query, [
                         'taxonomy' => $taxonomy,
                         'field' => 'slug',
...
//		}
		...
		if ( has_post_thumbnail() ) {
			echo '<div class="crt-grid-image-wrap" data-src="'. esc_url( $src ) .'"  data-img-on-hover="'. esc_attr( $settings['secondary_img_on_hover'] ) .'" data-src-secondary="'. esc_url( $src2 ) .'">';
				...
				if ( 'yes' == $settings['secondary_img_on_hover'] ) {
					echo '<img src="'. esc_url( $src2 ) . '" alt="'. esc_attr( $alt ) .'" class="crt-hidden-img crt-anim-timing-'. esc_attr($settings[ 'image_effects_animation_timing']) .'">';
				}
			echo '</div>';
		}
	}
	...
	public static function render_media_overlay( $settings ) {
		echo '<div class="crt-grid-media-hover-bg '. esc_attr(CRT_Woo_Grid_Helpers::get_animation_class( $settings, 'overlay' )) .'" data-url="'. esc_url( get_the_permalink( get_the_ID() ) ) .'">';
...
//			}
	...
	public static function render_product_categories( $settings, $class, $post_id, $general_settings ) {
		...
		// Pointer Class
		$categories_pointer = isset($general_settings['categories_pointer']) ? $general_settings['categories_pointer']:'';
		$categories_pointer_animation = isset($general_settings['categories_pointer_animation']) ? $general_settings['categories_pointer_animation']:'';
		$pointer_item_class = (isset($general_settings['categories_pointer']) && 'none' !== $general_settings['categories_pointer']) ? 'class="crt-pointer-item"' : '';

		$class .= ' crt-pointer-'. $categories_pointer;
		$class .= ' crt-pointer-line-fx crt-pointer-fx-'. $categories_pointer_animation;

		echo '<div class="'. esc_attr($class) .' crt-grid-product-categories">';
			echo '<div class="inner-block">';
				// Text: Before
	...
	public static function render_product_tags( $settings, $class, $post_id, $general_settings ) {
		...
		// Pointer Class
		$tags_pointer = $general_settings['tags_pointer'];
		$tags_pointer_animation = $general_settings['tags_pointer_animation'];
		$pointer_item_class = (isset($general_settings['tags_pointer']) && 'none' !== $general_settings['tags_pointer']) ? 'class="crt-pointer-item"' : '';

		$class .= ' crt-pointer-'. $tags_pointer;
		$class .= ' crt-pointer-line-fx crt-pointer-fx-'. $tags_pointer_animation;

		echo '<div class="'. esc_attr($class) .' crt-grid-product-tags">';
			echo '<div class="inner-block">';
				// Text: Before
				if ( 'before' === $settings['element_extra_text_pos'] ) {
					echo '<span class="crt-grid-extra-text-left">'. esc_html( $settings['element_extra_text'] ) .'</span>';
		...
		echo '<div class="'. esc_attr($class) .'">';
			...
			// Sale
			 if ( $product->is_on_sale() ) {
				echo '<span class="crt-woo-onsale">'. esc_html__( 'Sale', 'crt-manage' ) .'</span>';
			}

			// Stock Status
			if ( 'yes' === $settings['element_status_offstock'] && $product->is_in_stock() == false && 
				 ! ( $product->is_type( 'variable' ) && $product->get_stock_quantity() > 0 ) ) {
				echo '<span class="crt-woo-outofstock">'. esc_html__( 'Out of Stock', 'crt-manage' ) .'</span>';
			}

			// Featured
			if ( 'yes' === $settings['element_status_featured'] && $product->is_featured() ) {
				echo '<span class="crt-woo-featured">'. esc_html__( 'Featured', 'crt-manage' ) .'</span>';
			}
	...
	public static function render_product_price( $settings, $class ) {
		...
		echo '<div class="'. esc_attr($class) .'">';
			...
			$sale_price_dates_to = apply_filters( 'crt_custom_sale_price_dates_to_filter', $sale_price_dates_to, $product );
		...
		if ( ( 'yes' == $settings['show_sale_starts_date'] && !empty($sale_price_dates_from) ) || ( 'yes' == $settings['show_sale_ends_date'] && !empty($sale_price_dates_to) ) ) {
			echo '<div class="'. esc_attr($class) .'">';
				echo '<div class="inner-block">';
					...
					echo '<span class="crt-sale-dates">';
		
						// Text: Before
						if ( '' !== $settings['element_sale_starts_text'] && !empty($sale_price_dates_from) ) {
							echo '<span class="crt-grid-extra-text-left">'. esc_html( $settings['element_sale_starts_text'] ) .'</span> ';
						}
						...
						if ( '' !== $settings['element_sale_ends_text'] && !empty($sale_price_dates_to) ) {
							echo '<span class="crt-grid-extra-text-left">'. esc_html( $settings['element_sale_ends_text'] ) .'</span> ';
						}
		...
		echo '<div class="'. esc_attr($class . $style_class) .'">';
			echo '<div class="inner-block">';
				...
				if ( 'yes' === $settings['element_rating_score'] ) {
					...
					echo '<i class="crt-rating-icon-10">'. esc_html($rating_icon) .'</i>';
                    ...
                    if ( \Elementor\Plugin::$instance->experiments->is_feature_active( 'e_font_icon_svg' ) && 'style-1' == $settings['element_rating_style'] ) {
                        for ( $b = 1; $b <= 5;  $b++ ) {
                            ...
                            if ( $b <= $rating_amount ) :
                                CRT_Woo_Grid_Helpers::render_rating_icon( 'crt-rating-icon-full', $settings['element_rating_unmarked_style'] );
                            elseif ( $b === $round_rating + 1 && $rating_amount !== $round_rating ) :
                                CRT_Woo_Grid_Helpers::render_rating_icon( 'crt-rating-icon-'. (( $rating_amount - $round_rating ) * 10), $settings['element_rating_unmarked_style'] );
                            else :
                                CRT_Woo_Grid_Helpers::render_rating_icon( 'crt-rating-icon-0', $settings['element_rating_unmarked_style'] );
                            endif;
                        ...
                        for ( $i = 1; $i <= 5; $i++ ) {
                            ...
                            if ( $i <= $rating_amount ) {
                                ...
                                echo '<i class="crt-rating-icon-'. esc_attr((( $rating_amount - $round_rating ) * 10)) .'">'. esc_html($rating_icon) .'</i>';
                            } else {
	...
	public static function render_product_add_to_cart( $settings, $class, $general_settings ) {
		...
		$add_to_cart_animation = $general_settings['add_to_cart_animation'];
		...
		$attributes = [
			'rel="nofollow"',
			'class="'. esc_attr($button_class) .' crt-button-effect '. esc_attr($add_to_cart_animation) .' '. (!$product->is_in_stock() && 'simple' === $product->get_type() ? 'crt-atc-not-clickable' : '').'"',
		...
		if ( 'before' === $settings['element_extra_icon_pos'] ) {
			...
			$button_HTML .= '<span class="crt-grid-extra-icon-left">'. $extra_icon .'</span>';
		}
		...
		if ( 'simple' === $product->get_type() ) {
			...
			$button_HTML .= esc_html__('Select Amount', 'crt-manage');
			array_push( $attributes, 'href="'. esc_url( get_permalink() ) .'"' );
		} else if ( 'ywf_deposit' === $product->get_type() ) {
			$button_HTML .= esc_html__('Select Amount', 'crt-manage');
			array_push( $attributes, 'href="'. esc_url( get_permalink() ) .'"' );
		} else if ( 'stm_lms_product' === $product->get_type() ) {
			$button_HTML .= esc_html__('View Product', 'crt-manage');
			array_push( $attributes, 'href="'. esc_url( get_permalink() ) .'"' );
		...
		if ( 'after' === $settings['element_extra_icon_pos'] ) {
			...
			$button_HTML .= '<span class="crt-grid-extra-icon-right">'. $extra_icon .'</span>';
		}
...
//		}
		...
		if ($user_id > 0) {
			...
			$wishlist = CRT_Woo_Grid_Helpers::get_wishlist_from_cookie();
		}
		...
		$wishlist_attributes = [
			'data-wishlist-url' => get_option('crt_wishlist_page') ? get_option('crt_wishlist_page') : '',
		...
		echo '<div class="'. esc_attr($class) .'">';
			...
			echo '<button class="crt-wishlist-add '. $add_button_hidden .'" '. $button_add_title .' data-product-id="' . $product->get_id() . '"'. ' ' . implode( ' ', $wishlist_attributes ) .' >'. $add_to_wishlist_content .'</button>';
			echo '<button class="crt-wishlist-remove '. $remove_button_hidden .'" '. $button_remove_title .' data-product-id="' . $product->get_id() . '">'. $remove_from_wishlist_content .'</button>';
...
//		}
		...
		if ($user_id > 0) {
			...
			$compare = CRT_Woo_Grid_Helpers::get_compare_from_cookie();
		}
		...
		$compare_attributes = [
			'data-compare-url' => get_option('crt_compare_page') ? get_option('crt_compare_page') : '',
		...
		echo '<div class="'. esc_attr($class) .'">';
			...
			echo '<button class="crt-compare-add '. $add_button_hidden .'" '. $button_add_title .' data-product-id="' . $product->get_id() . '"'. ' ' . implode( ' ', $compare_attributes ) .' >'. $add_to_compare_content .'</button>';
			echo '<button class="crt-compare-remove '. $remove_button_hidden .'" '. $button_remove_title .' data-product-id="' . $product->get_id() . '">'. $remove_from_compare_content .'</button>';
	...
	public static function render_product_read_more( $settings, $class, $general_settings ) {
		$read_more_animation = 'crt-button-none';
		$open_links_in_new_tab = 'yes' === $general_settings['open_links_in_new_tab'] ? '_blank' : '_self';
...
//		}
		...
		switch ( $type ) {
			case 'title':
				CRT_Woo_Grid_Helpers::render_product_title( $settings, $class, $general_settings );
				break;

			case 'excerpt':
				CRT_Woo_Grid_Helpers::render_product_excerpt( $settings, $class );
				break;

			case 'product_cat':
				CRT_Woo_Grid_Helpers::render_product_categories( $settings, $class, $post_id, $general_settings );
				break;

			case 'product_tag':
				CRT_Woo_Grid_Helpers::render_product_tags( $settings, $class, $post_id, $general_settings );
				break;

			case 'likes':
				CRT_Woo_Grid_Helpers::render_product_likes( $settings, $class, $post_id );
				break;

			case 'sharing':
				CRT_Woo_Grid_Helpers::render_product_sharing_icons( $settings, $class );
				break;

			case 'lightbox':
				CRT_Woo_Grid_Helpers::render_product_lightbox( $settings, $class, $post_id );
				break;

			case 'separator':
				CRT_Woo_Grid_Helpers::render_product_element_separator( $settings, $class );
				break;

			case 'status':
				CRT_Woo_Grid_Helpers::render_product_status( $settings, $class );
				break;

			case 'price':
				CRT_Woo_Grid_Helpers::render_product_price( $settings, $class );
				break;

			case 'sale_dates':
				CRT_Woo_Grid_Helpers::render_product_sale_dates( $settings, $class );
				break;

			case 'rating':
				CRT_Woo_Grid_Helpers::render_product_rating( $settings, $class );
				break;

			case 'add-to-cart':
				CRT_Woo_Grid_Helpers::render_product_add_to_cart( $settings, $class, $general_settings );
				break;
			case 'wishlist-button':
                CRT_Woo_Grid_Helpers::render_product_wishlist_button( $settings, $class, $general_settings );
				break;
			case 'compare-button':
                CRT_Woo_Grid_Helpers::render_product_compare_button( $settings, $class, $general_settings );
				break;
			case 'custom-field':
				CRT_Woo_Grid_Helpers::render_product_custom_fields( $settings, $class, $post_id );
				break;

			case 'read-more':
				CRT_Woo_Grid_Helpers::render_product_read_more( $settings, $class, $general_settings );
			default:
				CRT_Woo_Grid_Helpers::render_product_categories( $settings, $class, $post_id, $general_settings );
				break;
		}
...
//			}
				...
				foreach ( $locations[$location] as $align => $elements ) {
					...
					echo '<div class="crt-grid-media-hover-'. esc_attr($align) .' elementor-clearfix">';
						foreach ( $elements as $data ) {
							...
							// Element
							CRT_Woo_Grid_Helpers::get_elements( $data['element_select'], $data, $class, $post_id, $settings );
						}
					echo '</div>';
	...
	public static function render_grid_pagination( $settings ) {
		...
		if ( 'yes' !== $settings['advanced_filters'] && 1 === CRT_Woo_Grid_Helpers::get_max_num_pages( $settings ) ) {
			return;
		}
		
		if ( (isset(CRT_Woo_Grid_Helpers::$my_upsells) && (count(CRT_Woo_Grid_Helpers::$my_upsells) <= $settings['query_posts_per_page'])) || (isset(CRT_Woo_Grid_Helpers::$crossell_ids) && (count(CRT_Woo_Grid_Helpers::$crossell_ids) <= $settings['query_posts_per_page'])) ) {
			return;
		}

		global $paged;
		$pages = CRT_Woo_Grid_Helpers::get_max_num_pages( $settings );
...
//		}
			    ...
			    if ( 'yes' === $settings['pagination_prev_next'] || 'yes' === $settings['pagination_first_last'] ) {
				    ...
				    if ( 'yes' === $settings['pagination_first_last'] ) {
				    	if ( $paged >= 2 ) {
					    	...
					    	// echo '<a href="'. esc_url(get_pagenum_link( $paged + 1, true )) .'" class="crt-first-page">';
					    	// echo '<a href="'. esc_url(substr(get_pagenum_link( $paged + 1, true ), 0, strpos(get_pagenum_link( $paged + 1, true ), '?orderby'))) .'" class="crt-first-page">';
		...
		// Load More / Infinite Scroll
			...
			// echo '<a href="'. esc_url_raw( str_replace( 999999999, '%#%', remove_query_arg( 'add-to-cart', get_pagenum_link( 999999999, false ) ) ) ) .'" class="crt-load-more-btn" data-e-disable-page-transition>';
			// echo '<a href="'. esc_url(get_next_posts_page_link()) .'" class="crt-load-more-btn" data-e-disable-page-transition>';
	...
	public function crt_get_filtered_count_products() {
		$nonce = $_POST['nonce'];
		...
		if ( isset($_POST['crt_url_params']) ) {
			...
			foreach ($_POST['crt_url_params'] as $params) {
				$query_args = CRT_Woo_Grid_Helpers::get_main_query_args($_POST['grid_settings'], $params);
	...
	public function crt_woo_grid_filters_ajax() {
		...
		// Get Posts
        $args = CRT_Woo_Grid_Helpers::get_main_query_args($settings, []);
		...
		while ( $posts->have_posts() ) : $posts->the_post();
			...
			// Content: Above Media
			CRT_Woo_Grid_Helpers::get_elements_by_location( 'above', $settings, get_the_ID() );
			...
			if ( has_post_thumbnail() ) {
				echo '<div class="crt-grid-media-wrap'. esc_attr(CRT_Woo_Grid_Helpers::get_image_effect_class( $settings )) .' " data-overlay-link="'. esc_attr( $settings['overlay_post_link'] ) .'">';
					...
					echo '<div class="crt-grid-media-hover crt-animation-wrap">';
						...
						// Content: Over Media
						CRT_Woo_Grid_Helpers::get_elements_by_location( 'over', $settings, get_the_ID() );
			...
			// Content: Below Media
			CRT_Woo_Grid_Helpers::get_elements_by_location( 'below', $settings, get_the_ID() );
