...
/**
 * CRT_Count_Wishlist_Compare_Items setup
 *
 * @since 1.0
 */
class CRT_Count_Wishlist_Compare_Items { 
    ...
    */
        ...
        foreach ($wishlist as $product_id) {
            ...
            if ($product) {
                $wishlist_product_array[] = [
                    ...
                    'product_stock' => $product->get_stock_status() == 'instock' ? esc_html__('In Stock', 'crt-addons') : esc_html__('Out of Stock', 'crt-addons'),
    ...
    public function compare_table() {
        ...
        if ( ! $compare ) {
			$table_hidden = 'crt-hidden-element';
        } else {
			$notification_hidden = 'crt-hidden-element';
		}

        ob_start();

		echo '<p class="crt-compare-empty '. $notification_hidden .'">'. $settings['compare_empty_text'] .'</p>';
        
        // Start the table
		echo '<div class="crt-compare-products '. $table_hidden .'">';
        echo '<table class="crt-compare-table">';
            ...
            foreach ( $compare as $product_id ) {
                ...
                $remove_text_tr = sprintf( __( '%s', 'crt-addons' ), $remove_text );
                
                echo '<th data-product-id="' . esc_attr( $product->get_id() ) . '">
                        <button class="crt-compare-remove" data-product-id="' . esc_attr( $product->get_id() ) . '">' . esc_html( $remove_text_tr ) . '</button>
                      </th>';
            ...
            $table_data = array(
                ['label' => esc_html__('Image', 'crt-addons'), 'type' => 'image'],
                ['label' => esc_html__('Name', 'crt-addons'), 'type' => 'text'],
                ['label' => esc_html__('Rating', 'crt-addons'), 'type' => 'text'],
                ['label' => esc_html__('Description', 'crt-addons'), 'type' => 'text'],
                ['label' => esc_html__('Price', 'crt-addons'), 'type' => 'text'],
                ['label' => esc_html__('SKU', 'crt-addons'), 'type' => 'text'],
                ['label' => esc_html__('Stock Status', 'crt-addons'), 'type' => 'text'],
                ['label' => esc_html__('Dimensions', 'crt-addons'), 'type' => 'text'],
                ['label' => esc_html__('Weight', 'crt-addons'), 'type' => 'text']
            );
            ...
            foreach ( $table_data as $row ) {
                ...
                foreach ( $compare as $key => $product_id ) {
                    ...
                    switch ( $row['type'] ) {
                        case 'image':
                            echo '<a class="crt-compare-img-wrap" href="' . $product->get_permalink() . '">' . $product->get_image() . '</a>';
							echo '<div class="crt-compare-product-atc">' . $this->render_product_add_to_cart( $settings, $product ) . '</div>';
                            break;
                        case 'text': 
                            if( in_array(strtolower($row['label']), [strtolower(__('Description', 'crt-addons')), strtolower(__('SKU', 'crt-addons')) ]) ) {
                                echo $product->get_data()[strtolower($row['label'])];
                            } else if ( strtolower($row['label']) == strtolower(__('Name', 'crt-addons')) ) {
                                echo '<a class="crt-compare-product-name" href="' . $product->get_permalink() . '">'. $product->get_data()['name'] .'</a>';
							} else if ( strtolower($row['label']) == strtolower(__('Price', 'crt-addons')) ) {
                                echo $product->get_price_html();
                            } else if ( strtolower($row['label']) ==strtolower(__('Rating', 'crt-addons')) ) {
                                echo $this->render_product_rating($product);
                            } else if ( strtolower($row['label']) == strtolower(__('Stock Status', 'crt-addons')) ) {
					            $stock_status = $product->get_stock_status();
                                echo $stock_status == 'instock' ? esc_html__('In Stock', 'crt-addons') : esc_html__('Out of Stock', 'crt-addons');
                            } else if ( strtolower($row['label']) == strtolower(__('Dimensions', 'crt-addons')) ) {

								if ( $product->has_dimensions() ) {
									$dimensions = sprintf(
										'<span class="crt-dimensions">%s</span>',
							...
							} else if ( strtolower($row['label']) == strtolower(__('Weight', 'crt-addons')) ) {

								if ( $product->get_weight() ) {
									$weight = sprintf(
										'<span class="crt-weight">%s %s</span>',
										$product->get_weight(),
...
new CRT_Count_Wishlist_Compare_Items();