...
/**
 * CRT_Add_Remove_From_Compare setup
 *
 * @since 1.0
 */
class CRT_Add_Remove_From_Compare { 
    ...
    */
    ...
    //     $args = array(
    //         'label'				  => esc_html__( 'Royal compare', 'crt-addons' ),
    //         'public'              => true,
    ...
    function add_to_compare() {
        ...
        if ( ! wp_verify_nonce( $nonce, 'crt-addons-js' ) || ! isset( $_POST['product_id'] ) ) {
            return;
        }
        ...
        //     wp_send_json_error( array( 'message' => esc_html__('Product is already in Compare.', 'crt-addons') ) );
        //     return;
        // }
        // $compare[] = $product_id;
        ...
        if ($user_id > 0) {
            update_user_meta($user_id, 'crt_compare', $compare);
        } else {
    ...
    function set_compare_to_cookie($compare) {
        if ( is_multisite() ) {
            setcookie('crt_compare_'. get_current_blog_id() .'', json_encode($compare), time() + (86400 * 10), '/'); // Expires in 7 days
        } else {
            setcookie('crt_compare', json_encode($compare), time() + (86400 * 10), '/'); // Expires in 7 days
        }
    }
}

new CRT_Add_Remove_From_Compare();