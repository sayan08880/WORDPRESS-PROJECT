...
/**
 * CRT_Compare_Popup_Action setup
 *
 * @since 1.0
 */
class CRT_Compare_Popup_Action { 

    public function __construct() {
        add_action('wp_ajax_crt_get_page_content', [$this, 'crt_get_page_content']);
        add_action('wp_ajax_nopriv_crt_get_page_content', [$this, 'crt_get_page_content']);
        // add_action('rest_api_init', [$this, 'register_compare_custom_routes']);
    }
    
    // function register_compare_custom_routes() {
    //     register_rest_route('crt-addons/v1', '/page-content/(?P<id>\d+)', array(
    //         'methods' => 'GET',
    //         'callback' => 'crt_get_page_content'
    //     ));
    // }

    function crt_get_page_content($request) {

        $page_id = $_POST['crt_compare_page_id'];

        if ( post_password_required($page_id) || 'publish' !== get_post_status($page_id) ) {
            wp_send_json_error(array(
                'message' => esc_html__('Security check failed.', 'crt-addons'),
            ));
        }

		if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'crt-addons-js')) {
			wp_send_json_error(array(
				'message' => esc_html__('Security check failed.', 'crt-addons'),
			));
		}
