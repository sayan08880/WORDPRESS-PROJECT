(function($) {
    ...
    $(window).on( 'elementor/frontend/init', function() {
        elementorFrontend.hooks.addAction('frontend/element_ready/crt-product-filters.default',function($scope) {
            ...
            function renderActiveFilters( $origin = '' ) {
                ...
                if ( activeFilters ) {
                    if ( $scope.find('.crt-af-active-filters').length > 0 ) {
                    ...
                    $('.crt-advanced-filters-wrap').each(function () {
                        ...
                        if (thisWrap.find('.crt-af-rf-control').length > 0) {
                            var rangeValues = $(this).find('.crt-af-rf-control input[type="number"]').map(function () {
                                return $(this).val();
                            }).get();
                    ...
                    $('body').find('.crt-af-range-container').each(function() {
                        var rangeContainer = $(this),
                            ...
                            minInput = rangeContainer.find('input.crt-af-rf-control-min-input'),
                            maxInput = rangeContainer.find('input.crt-af-rf-control-max-input'),
                ...
                if ( $('.crt-af-apply-btn').length == 0 ) {
                    ...
                    if ( $scope.find('.crt-af-range-apply-btn').length > 0 ) {
                        $scope.on('click', '.crt-af-range-apply-btn', function() {
                            var selectedElement = $scope.find('.crt-af-rf-control').find('input[type="number"]');
                            var selectedValue = [];
                        ...
                        $scope.find('.crt-af-range-container').find('input[type="range"], input[type="number"]').on('change', function() {
                            var selectedElement = $scope.find('.crt-af-rf-control').find('input[type="number"]');
                            var selectedValue = [];
                            var selectedName = selectedElement.attr('name');
            ...
            function controlFromSlider(fromSlider, toSlider, fromInput) {
                ...
                if (from > to) {
                    fromSlider.val(to).attr('value', to);
                    fromInput.val(to).attr('value', to);
                    $scope.find('.crt-af-rs-value-min').text(to);
                } else {
                    fromSlider.val(from).attr('value', from);
                    fromInput.val(from).attr('value', from);
                    $scope.find('.crt-af-rs-value-min').text(from);
                }
            }
            ...
            function controlToSlider(fromSlider, toSlider, toInput) {
                ...
                if (from <= to) {
                    toSlider.val(to).attr('value', to);
                    toInput.val(to).attr('value', to);
                    $scope.find('.crt-af-rs-value-max').text(to);
                } else {
                    toInput.val(from).attr('value', from);
                    toSlider.val(from).attr('value', from);
                    $scope.find('.crt-af-rs-value-max').text(from);
                }
            }
                ...
                $scope.on('click', '.crt-af-reset-btn', function() {
                    ...
                    if ( 'yes' !== $(this).closest('.crt-advanced-filters-wrap').data('enable-ajax') ) {
                        ...
                        $('body').find('.crt-advanced-filters-wrap .crt-af-range-container').each(function() {
                            let rangeMin = $(this).find('input[type="number"].crt-af-rf-control-min-input'),
                                rangeMax = $(this).find('input[type="number"].crt-af-rf-control-max-input'),
                                thisFromSlider = $(this).find('input[type="range"].crt-af-from-slider'),
                                thisToSlider = $(this).find('input[type="range"].crt-af-to-slider'),
                                thisFromSliderText = $(this).find('.crt-af-rs-value-min'),
                                thisToSliderText = $(this).find('.crt-af-rs-value-max');
            ...
            function ajaxFilters(self = '') {
                ...
                $('.crt-advanced-filters-wrap').each(function() {
                    ...
                    if ( $(this).find('.crt-af-range-apply-btn').length > 0 && $(this).data('crt-applied') == 'yes' ) {
                        var selectedElement = $(this).find('.crt-af-rf-control').find('input[type="number"]');
                        var selectedValue = [];
                        var selectedName = selectedElement.attr('name');
        ...
        var CrtElements = {
            ...
            mediaHoverLink: function ($scope, iGrid) {
                ...
                if ( 'yes' === iGrid.find( '.crt-grid-media-wrap' ).attr( 'data-overlay-link' ) && ! CrtElements.editorCheck() ) {
