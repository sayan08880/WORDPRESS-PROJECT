				...
				$.ajax(
					{
						...
						success: function (response) {
							...
							if (undefined !== page_content && '' !== page_content) {
								...
								var elements = Array.isArray(page_content) ? page_content : [page_content];

								console.log('Woostify Sites: Elements to import', elements);
								if (elements.length === 0 || (elements.length === 1 && !elements[0])) {
									console.error('Woostify Sites: No valid elements found to import');
									alert('No valid Elementor content found in this block.');
									return;
								}
								...
								try {
									if (undefined !== $e && 'undefined' !== typeof $e.run) {
										// Use $e.run for modern Elementor
										...
										// Fallback for older Elementor or if $e.run fails
										$.each(elements, function (i, element) {
											if (!element.elType) {
												element.elType = element.isInner ? 'column' : 'section';
											}
											elementor.getPreviewView().addChildModel(element, { at: index++ });
										});
									}
								} catch (err) {
									console.error('Woostify Sites: Import Error', err);
									// Final fallback
									$.each(elements, function (i, element) {
										elementor.getPreviewView().addChildModel(element, { at: index++ });
									});
								...
								if (undefined !== elementor.saver && 'undefined' !== typeof elementor.saver.setFlagEditorChange) {
			...
			$.ajax(
				{
					...
					success: function (response) {
						$('#woostify-sites-modal').css('display', 'flex');
						$('#woostify-sites-modal').css('justify-content', 'center');
						$('#woostify-sites-modal').html(response);
						if (tab) {
